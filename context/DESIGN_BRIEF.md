...
Two live-app-derived directions were compared. A native select placed permanently in the header is the fewest-code option, but it grows the header from 160px to 241px at 1440×900, 158px to 239px at 390×844, 145px to 221px at 844×390, and 158px to 239px at 320×568. It also turns a map introduction into a standing form. The selected direction is instead one visible **Places** action in the existing Explore header. It preserves the 160px desktop and 145px landscape headers, but its third action honestly wraps the 390×844 and 320×568 headers from 158px to 209px: a 51px cost, still 30px shorter than the permanent-select concept. It opens the existing dock only on request, with the exact title **Places**, a native 44px **Pacific place** select, and a plain statement that alphabetical order is for finding—not ranking need, readiness, or vulnerability. Desktop reserves the existing 400px only while the picker is open; compact layouts use the existing expanded sheet and body scrolling. Matched concepts and measurements live under `artifacts/design/task-117/`.

The picker is an ephemeral disclosure, not a route or second explorer. Its still-visible trigger owns `aria-expanded={placesOpen}` and `aria-controls="explorer-panel-dock"`; the on-demand dock uses that stable ID. Opening writes no history and focuses the native select. Reactivating the trigger toggles the panel closed and deterministically retains/restores trigger focus; panel Close also restores the trigger. Choosing a loaded stable code first clears the disclosure without restoring trigger focus, calls the existing `handleSelect`, and focuses the selected panel's Close control so removed form focus cannot fall to the document. One local `closePlaces` helper—not a state machine—must clear `placesOpen` before every canonical transition away: map/`handleSelect`, score choice, **Data coverage**, **Rank ranges**, the 2030 stress test, **Guided tour**, **Methods & sources**, and popstate/Back-Forward. Popstate clears before canonical URL hydration. Destination controls keep their existing focus ownership, and the disclosure cannot reappear stale when state later returns to neutral. Direct selected URLs and diagnostic root/child Back/Close behavior remain unchanged; the Places action is shown only when no selected or diagnostic panel owns the dock. Loading/error/zero-loaded-record states remain owned by the current app-wide gate, so no empty or disabled picker is invented.

TASK-117 changes no runtime file. TASK-118 is the single bounded implementation successor and begins only after the held TASK-110/TASK-100 release gates. It may add one local disclosure boolean, trigger ref, and clearing helper through existing `App.tsx`, panel, CSS, and test seams, but no custom combobox, search dependency, URL parameter, alternate selected code, disclosure state machine, panel stack, drawer, camera behavior, data change, or dashboard shell.
