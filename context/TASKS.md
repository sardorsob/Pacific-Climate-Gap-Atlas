...
- Depends on: TASK-095, TASK-101, TASK-102, TASK-103, TASK-104, TASK-110, TASK-116, TASK-119
...
- QA notes: Attempt 1 automated and independent frozen-production QA passed 802 usable assertions and 10 inspected frames, including exact source/group reproduction, all inspection inputs, seven accepted viewports, reflow, color/motion variants, history, accessibility, and the repaired TASK-102 map-failure state. Owner review did not accept the lens as finished and returned the bounded Direction A+ correction. TASK-111 and TASK-112 have now passed independent review: the current baseline has the approved copy/rhythm, five distinct reference/inspection/selection identities, selected-self wording, and at least 44.37px bands at the 320px floor. Attempt 2 regenerated the final TASK-114/TASK-115 frames and passed automated gates, but independent review rejected closure: the 1024x768 selected-place map-failure notice was partly hidden behind the desktop legend, and the provenance summarized interactions without preserving the required before/after state matrix or exact contrast measurements. TASK-119 independently accepted the one-rule desktop failure-only legend repair. Attempt 3 regenerated the ten-frame set and retains 13 complete interaction before/after records, 14 responsive/accessibility cases, 12 exact contrast measurements, and 10 explicit source-to-model-to-sentence-to-DOM cases. Its first independent review found that the selected-null keyboard record sampled a still-moving panel-expansion camera before the action, and that the retained portrait visibility-inspection frame did not actually show its plot/cursor/status. The evidence-only correction proves the discrepancy was harness timing: after waiting for MapLibre idle, asserting `isMoving() === false`, two animation frames, and two 50ms-separated stable samples, Pitcairn remains at center `[-128.3, -24.4]`, zoom `1.38`, and selected target `(195, 422)` before and after ArrowRight. Every delta is zero against `0.000001°`, `0.000001` zoom, and `0.01px` target tolerances. The recaptured 390x844 Fiji frame visibly contains the 14-of-14 plot, 46x44 inspection cursor, 12-of-22 status, and exact visible/full inspector text. All 49 structured cases pass with zero console, page, request, overflow, or map-alert intersection failures. Independent re-review resolved both original findings and returned Spec PASS / quality Approved with one non-blocking cleanup: the Fiji case's unused source-derived camera fallback now names Fiji `[178.1, -17.7]` instead of Pitcairn. Production, tests, data, dependencies, routes, and state remain frozen at `6a847f5`; the owner accepted the live result on 2026-08-18.
- Attempts: 3
- Max attempts: 3
- Attempt log: 2026-08-02: TASK-110 moved `pending -> in-progress` after TASK-109 passed independent scientific/visualization/accessibility review. Production was frozen at `83b52a1`; QA owned only bounded design/provenance evidence and active task/handoff records unless a reproduced defect was formally returned to an owning implementation task. The first pass returned one responsive map-failure defect to TASK-102. After TASK-102 attempt 3 closed at `069e811`, QA resumed without changing production, superseded the affected assertion/frame, justified reuse of unaffected evidence against the one-rule diff, passed the fresh repair/inactivity slice and full gates, and moved `in-progress -> in-review`. Owner review then returned the typography/rhythm and narrow-width interaction findings above; TASK-110 moved `in-review -> needs-fix` and delegated the repair to TASK-111 and TASK-112. 2026-08-18: after TASK-114/TASK-115 landed, attempt 2 moved `needs-fix -> in-progress -> in-review` at `9086443`. It regenerated all ten frames rather than reusing changed pixels and passed the full automated suite, but independent review found the selected-place failure notice occluded by the desktop legend and the interaction evidence insufficiently auditable. TASK-110 moved `in-review -> needs-fix`; scientific values, direction groups, clocks, null handling, visibility groups, frozen scope, and no-co-author history were accepted. TASK-119 owned and independently closed the one-seam failure-state repair. Attempt 3 legally moved `needs-fix -> in-progress -> in-review` against frozen accepted head `6a847f5`, regenerated the affected evidence, retained complete case-level interaction/accessibility/contrast/scientific records, passed all 49 recorded cases plus full repository gates, and changed no production/test/data/dependency/route/state file. Independent review then returned two evidence-only findings: an in-flight camera pre-sample and an off-frame portrait inspection. Attempts remain 3 while the correction legally records `in-review -> needs-fix -> in-progress -> in-review`; stable idle/two-rAF camera evidence and the verified in-frame screenshot replace only the two rejected records. Independent scientific/UX/accessibility re-review resolved both findings and returned Spec PASS / quality Approved; its sole non-blocking metadata cleanup aligns the unused Fiji fallback coordinate. The owner accepted the live result on 2026-08-18, moving TASK-110 `in-review -> done`.
- Status: done
...
- QA notes: Strict RED failed 1/26 focused assertions because `HANDOFF_COPY` still had the retired instruction; one-constant GREEN passed focused 26/26 and the story regression passed 29/29. Removing the refusal clause after the semicolon failed the exact-copy assertion, then restoration returned the suite to green. A fix-round RED exposed the shipped map-fallback alert collision: the sticky map’s viewport-sized `role=alert` showed through the transparent guided handoff. One local `:has()` rule now isolates that existing alert only in guided mode, retaining it in the DOM and restoring visible opacity on Explore. Final batch review then reproduced the true loaded-story 132px sticky chrome at 844x390 and found the eyebrow hidden by it; the earlier artifact path had skipped story-chrome mounting. Fix round 2 keeps type, copy, state, and action unchanged, adds only landscape handoff clearance and removes the landscape copy-width cap. Regenerated desktop/portrait/landscape frames and trace measure the eyebrow 15.5px below the 132px chrome, the complete copy visible, the action at 44px, zero overflow, and identical geometry with the accessible fallback alert present. Scoped batch re-review returned spec PASS and quality Approved with no finding; final focused tests pass 38/38 and full frontend tests pass 145/145.
- Attempts: 2
- Max attempts: 3
- Attempt log: 2026-08-18: TASK-116 moved `pending -> in-progress`; strict exact-copy RED failed as intended, the existing constant alone was replaced, and the refusal-clause mutation failed before restoration. Task then moved `in-progress -> in-review` with durable quiet-headless desktop/portrait/landscape frames and state trace under `artifacts/design/task-116/`. Fix round 1 isolated the real map-fallback collision with one guided-only CSS rule; new frames/trace verified the alert is accessible but hidden during the story and visible after Explore. Independent re-review accepted the conclusion/failure-state seam and TASK-116 moved `in-review -> done`. Final batch review returned the loaded 844x390 sticky-chrome collision; attempt 2 moved `done -> needs-fix -> in-progress -> in-review`, corrected only the responsive handoff seam at `f24be99`, and regenerated all three retained frames from the real story path. The scoped batch re-review found the loaded/fallback landscape geometry safe and all prior TASK-116 findings resolved, so TASK-116 moved `in-review -> done`.
- Status: done
...
## TASK-119
- Phase: map-failure-legend-clearance
- Title: Keep the map-failure notice clear of the desktop legend
- Depends on: TASK-102, TASK-110 attempt 2 review
- Assigned agent: Failure-state UI Builder; independent accessibility/responsive Checker
- Contract refs: TASK-102, TASK-110, context/DESIGN_BRIEF.md, context/ARTISTIC_REDESIGN_BRIEF.md
- Data refs: none; existing generated/public data frozen
- Scientific refs: none; the failure notice makes no analytical claim
- User value / decision value: Keeps the only visible failure explanation readable when a selected-place panel and the always-open desktop legend share the map region.
- Functional notes: In Explore map-failure states, prevent the desktop legend from covering the centered in-region alert. Prefer the smallest existing CSS/DOM seam; hiding a legend that explains absent marks is acceptable and avoids inventing another layout system. Preserve the selected panel, header, controls, Methods, mobile legend behavior during normal map operation, guided fallback isolation, map lifecycle, copy, routes, state, data, dependencies, and Night Watch styling.
- Statistical notes: None. Do not expose score, evidence, presence, or map-context claims when the map is unavailable.
- Edge cases: selected and unselected Explore failure states at 881, 1024, 1229, 1230, and 1440 widths; 390x844 and 844x390; guided mode; normal map state; panel open/closed; legend open/closed on mobile.
- Test cases: Begin with a focused failure proving the legend remains visible beside a map-failure alert. Add the smallest selector contract and quiet headless geometry check showing the alert and all retained chrome have zero overlap while the normal-map legend remains unchanged. Mutate the failure selector to prove the contract fails.
- Files to create/modify: app/src/styles/base.css; app/src/components/map/mapPresentation.test.tsx; artifacts/design/task-119/*; context/TASKS.md; context/logs/Progress Log.md; context/logs/Handoff Notes.md
- Artifacts to produce: selected and unselected 1024x768 failure frames; normal/failure visibility and alert/chrome overlap trace
- Acceptance criteria: The failure heading is fully visible and has zero overlap with the legend/header/controls/panel at every required layout; the desktop legend remains present during normal map operation; mobile and guided behavior remain correct; no lifecycle, component, state, route, data, dependency, copy, or unrelated visual change enters; focused/full/build/bundle/status/secret/whitespace gates and independent review pass.
- Verification commands: npm --prefix app run test -- mapPresentation.test.tsx; npm --prefix app run test; npm --prefix app run build; python scripts/check_app_bundle_budget.py; python scripts/validate_task_statuses.py; python scripts/check_secrets.py; git diff --check
- Manual QA: Quietly inspect selected/unselected Explore failure and normal-map states at desktop, portrait, and landscape; measure alert/chrome intersections and confirm no visible external browser opens.
- QA notes: Registered from TASK-110 attempt-2 independent review after the retained 1024x768 frame reproduced legend/alert occlusion. TASK-102 is complete at its declared three-attempt limit, so this bounded successor owns the new regression without weakening TASK-110's production freeze. Strict RED failed the new focused selector contract exactly because the desktop failure-only legend exclusion was absent; GREEN passed 8/8 after one existing-seam CSS rule, and mutating its Explore selector to guided failed again before restoration returned GREEN. Quiet headless Chromium passed 28/28 normal/failure selected/unselected cases across 881, 1024, 1229, 1230, and 1440 desktop plus 390x844 and 844x390: all alert/header/controls/panel/legend intersections and horizontal overflow were zero; failure hides the legend in all 10 desktop cases, preserves it in all 4 mobile failures, and normal desktop keeps it in all 10 cases. The selected and unselected 1024x768 failure frames and structured trace were visually inspected. Guided failure remains visually isolated (alert opacity 0), with no overflow. There were zero runtime or request errors. Independent re-review found no Critical, Important, or Minor issue, accepted the one-rule production fix and corrected desktop-block test, recomputed every trace count, and returned spec PASS / accessibility-responsive quality Approved / Ponytail **Lean already. Ship.**
- Attempts: 2
- Max attempts: 3
- Attempt log: 2026-08-18: TASK-119 moved pending -> in-progress before the focused failure-state legend contract. Attempt 1 added the smallest source contract, recorded strict RED at 7/8 because the selector was absent, added one desktop Explore `:has()` rule that hides only the legend beside the existing in-region alert, recorded GREEN at 8/8, mutated Explore -> guided to reproduce the focused failure, and restored 8/8 GREEN. Quiet/headless visual QA retained selected and unselected 1024x768 failures plus the 28-case normal/failure matrix and zero-error geometry trace. Focused and full frontend, build, bundle, task-status, required-artifact, secret, and whitespace gates pass. TASK-119 moved in-progress -> in-review for independent accessibility/responsive review; TASK-110 remains needs-fix pending its separate attempt 3 and fresh evidence. Independent review returned TASK-119 in-review -> needs-fix because the source contract could span the desktop media closing brace and its trace did not distinguish the guided record from the 28-case Explore matrix. Attempt 2 moved needs-fix -> in-progress before the fixture-driven contract correction; production CSS remains accepted and frozen. The moved-selector fixture passed the old broad contract and failed the extracted desktop-block contract exactly as expected; the corrected test now extracts that block, asserts the selector there, and asserts one full-source occurrence. The trace now labels the 28-case Explore matrix separately from its guided out-of-matrix result. TASK-119 moved in-progress -> in-review for independent accessibility/responsive re-review. Re-review found both prior findings resolved and no new issue, so TASK-119 moved in-review -> done. TASK-110 attempt 3 may begin from the accepted corrected baseline.
- Status: done

## TASK-120
- Phase: opening-atlas-actions
- Title: Add direct map and repository actions to the opening story header
- Depends on: TASK-057, TASK-116
- Assigned agent: Atlas UI Builder
- Contract refs: Owner-approved 2026-08-19 opening-action design; existing `handleExplore` and story handoff
- Data refs: none
- Scientific refs: none; these controls make no analytical claim
- User value / decision value: Lets readers skip directly to the interactive map or inspect the public project source without scrolling through the full guided story.
- Functional notes: Preserve **Pacific Climate Evidence Atlas** as opening identity while replacing its compass with Lucide's map icon. In guided mode only, add compact **View map** and **View GitHub** actions to the existing map-header action row. **View map** must reuse `handleExplore`, exactly like the final story handoff; **View GitHub** must be a normal external link to `https://github.com/sardorsob/Pacific-Climate-Gap-Atlas`, open in a new tab, and carry safe opener isolation. Give the opening map action and final **Explore the map** action the same Lucide map icon; give the repository action Lucide's GitHub icon. Reuse the existing `ghost-btn`, `map-header__actions`, Night Watch palette, and installed `lucide-react` package. Do not add a dependency, route, state variable, transition, tracking hook, or duplicate navigation function. Existing Explore-mode **Guided tour** and **Methods & sources** actions remain unchanged.
- Statistical notes: none
- Edge cases: Opening premise at desktop, portrait, landscape, and 320px; transition from opening action; transition from final handoff; external-link destination/security semantics; guided chrome layering after the premise; Explore header actions; keyboard focus and 44px target behavior; no horizontal overflow.
- Test cases: Begin with focused RED source/render assertions for the guided-only action row, exact labels, shared `handleExplore`, exact GitHub URL, `_blank` target plus opener isolation, Lucide map/GitHub icons, and a map icon in the final handoff. Keep existing story keyboard/control exclusion and App transition tests green.
- Files to create/modify: app/src/App.tsx; app/src/App.test.tsx; app/src/components/story/StoryScrolly.tsx; app/src/components/story/StoryScrolly.test.tsx; context/TASKS.md; context/PROJECT.md; context/HANDOVER.md; context/logs/Progress Log.md; context/logs/Handoff Notes.md
- Artifacts to produce: none; retain quiet visual measurements in the task log if layout needs correction
- Acceptance criteria: Both opening actions are visible on the premise; **View map** enters the same neutral Explore state as the final handoff; **View GitHub** reaches the exact public repository safely; map and GitHub icons are decorative and paired with visible text; the final handoff gains the matching map icon; current styling, responsive behavior, Guided/Explore behavior, story sequence, and map remain unchanged; focused/full/build/status/secret/whitespace gates pass.
- Verification commands: npm --prefix app run test -- App.test.tsx StoryScrolly.test.tsx; npm --prefix app run test; npm --prefix app run build; python scripts/check_app_bundle_budget.py; python scripts/validate_task_statuses.py; python scripts/check_secrets.py; git diff --check
- Manual QA: Quietly inspect the opening header at desktop, portrait, landscape, and 320px; activate both map entrances; confirm the external link's destination without opening a visible browser; verify no overlap, overflow, or change to Explore-mode actions.
- QA notes: The owner approved the exact bounded design before implementation. The repository remote supplies the canonical GitHub URL, and `lucide-react` already supplies `Map` and `Github`, so no package change was needed. Strict opening/handoff RED failed 2/26 exactly before production. The first GREEN run exposed a JavaScript/Lucide `Map` name collision in `StoryScrolly`; aliasing only the icon fixed the root cause and restored 26/26 without weakening the tests. Quiet hidden-browser inspection then found the taller opening header overlapping the premise at 320x568 and 844x390. A focused compact-clearance contract failed 1/12 before one active-premise-only 8rem top-padding rule and passed 12/12 afterward. Fresh frames at 2048x1080, 390x844, 320x568, and 844x390 show both 44px actions in one row, clear story content, retained Night Watch styling, and no visible page-width overflow. Final gates pass 148/148 frontend and 100/100 Python tests, production build at 1,046,533-byte JavaScript and 96,945-byte CSS, bundle/data/status/artifact/secret/Ruff/compile/whitespace checks.
- Attempts: 1
- Max attempts: 3
- Attempt log: 2026-08-19 attempt 1: TASK-120 moved `pending -> in-progress` after owner approval. Context and logs were updated before production or test changes. Strict RED/GREEN added the opening actions and handoff icon; systematic debugging resolved the icon/native-constructor collision with one alias. Quiet responsive inspection returned the compact overlap to focused RED/GREEN and accepted the active-premise-only clearance at all four viewports. Full verification passed, so TASK-120 moved `in-progress -> done` before its single no-co-author commit.
- Status: done

## TASK-121
- Phase: opening-action-hit-target-repair
- Title: Restore top-layer interaction for the opening map and GitHub actions
- Depends on: TASK-120
- Assigned agent: Atlas UI Repairer
- Contract refs: TASK-120; owner interaction report on 2026-08-26; existing `handleExplore` and repository link
- Data refs: none
- Scientific refs: none
- User value / decision value: Makes the two opening choices behave like the visible controls they already appear to be, so readers can leave the storyboard immediately for the map or source repository.
- Functional notes: Preserve the exact TASK-120 title, actions, icons, handlers, URL, Night Watch styling, and compact clearance. Move the existing `map-header` out of `.atlas-map-region` / `.guided-map`'s `z-index: 0` stacking context to the shared `.guided-atlas` layer, above the `z-index: 8` storyboard. Keep the header mounted throughout Explore, but in Guided mode mount it only while `sceneIndex === 0`; later scenes continue to own their existing sticky story chrome. Give only the opening guided header the z-index needed to sit above the story hit plane. Do not add pointer-event pass-through, duplicated controls, state, a component abstraction, route, handler, dependency, or map-interaction exception.
- Statistical notes: none
- Edge cases: Mouse, touch, and keyboard activation of **View map**; exact **View GitHub** destination and new-tab behavior; hit target at button centers; first guided scene at 1440x900, 390x844, 320x568, and 844x390; scroll into scene 2; Guided-tour re-entry; Explore **Guided tour** and **Methods & sources** actions; map interaction and story scrolling; no overlap or horizontal overflow.
- Test cases: Begin with a focused RED contract proving the opening header is outside the guided-map stacking context, is rendered only for Explore or guided scene zero, and retains the exact existing actions. Then use hidden Chromium to prove both centers resolve to their own button/link, **View map** changes the shell to Explore, the GitHub action opens the exact repository URL, later guided scenes remove the opening header, and Explore header actions remain reachable.
- Files to create/modify: app/src/App.tsx; app/src/App.test.tsx; app/src/styles/base.css; context/TASKS.md; context/PROJECT.md; context/HANDOVER.md; context/logs/Progress Log.md; context/logs/Handoff Notes.md
- Artifacts to produce: Temporary before/after opening screenshots and browser hit/click trace; no retained production asset required
- Acceptance criteria: At the opening storyboard, both visible action centers are the actual interactive controls; **View map** immediately enters neutral Explore through `handleExplore`; **View GitHub** opens the exact repository safely; the opening card looks active and remains clear at all four viewports; later guided scenes and all Explore header behavior are unchanged; no map click-through, duplicate UI, new state, dependency, route, handler, or unrelated styling enters; focused/full/build/bundle/data/status/artifact/secret/Ruff/compile/whitespace gates pass before commit.
- Verification commands: npm --prefix app run test -- App.test.tsx StoryScrolly.test.tsx; npm --prefix app run test; npm --prefix app run build; python -m pytest -q; python scripts/check_app_bundle_budget.py; python scripts/validate_data_contracts.py; python scripts/validate_task_statuses.py; python scripts/check_required_artifacts.py; python scripts/check_secrets.py; ruff check analysis scripts tests; python -m compileall -q analysis scripts tests; git diff --check
- Manual QA: Use hidden Chromium at all four viewports. Record `elementFromPoint` at both action centers, activate **View map**, capture and close the GitHub popup after verifying its URL, scroll to the next scene, re-enter Guided, and confirm no visible browser tab is left open.
- QA notes: Reproduced before edits at 1440x900. Both visible controls are geometrically 44px high, but `document.elementFromPoint` returns `SECTION.story-scene` at each center; Chromium times out clicking **View map** and the shell remains `atlas-shell--guided`. The root cause is the header's placement inside the sticky map's `z-index: 0` stacking context beneath sibling `.story-scrolly` at `z-index: 8`, not either handler or control markup. Strict RED failed 2/17 because the header was neither outside the map context nor above the story hit plane; GREEN passed focused App/story 29/29 after moving the unchanged markup and adding one guided z-index. Hidden Chromium at 1440x900, 390x844, 320x568, and 844x390 then resolved both 44px centers to the actual `BUTTON` and `A`. Mouse, keyboard Enter, and touch tap each moved **View map** to `atlas-shell--explore`; Explore retained one **Guided tour** and one **Methods & sources** control. **View GitHub** opened exactly `https://github.com/sardorsob/Pacific-Climate-Gap-Atlas` and the popup was closed. Scene two removed the opening action while retaining one story chrome. Three after frames were inspected and show the card undimmed, clear of story copy, and inside each viewport. Final gates pass frontend 150/150, Python 100/100, build at 1,046,558-byte JavaScript and 96,956-byte CSS, bundle/data/status/artifact/secret/Ruff/compile/whitespace checks.
- Attempts: 1
- Max attempts: 3
- Attempt log: 2026-08-26 attempt 1: TASK-121 moved `pending -> in-progress` after exact hidden-Chromium reproduction and source/stacking trace. Context and logs were updated before test or production changes. Strict RED/GREEN moved only the existing header and raised only its guided stacking level. Four-view mouse/hit evidence plus keyboard, touch, popup, later-scene, and Explore checks passed, followed by all repository gates. TASK-121 moved `in-progress -> done` before one no-co-author commit.
- Status: done
