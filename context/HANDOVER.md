...
The repository is a context-first GIS/data-science project. `TASK-001` through `TASK-094` and TASK-096 through TASK-099 are complete. On 2026-07-23 the owner approved TASK-090's Night Watch contract, TASK-091 established its budget-neutral surface foundation, TASK-092 carried the hierarchy through the guided story, TASK-093 completed the Explorer treatment while deleting the old glow machinery, and TASK-094 passed the integrated responsive/accessibility refinement. Frozen-production TASK-095 has passed automated and independent QA and remains `in-review` for owner visual/interaction acceptance. The owner-approved inline regional lens is implemented and independently approved. Manual QA then registered TASK-101 through TASK-104 for source/copy correctness, map-failure resilience, live-breakpoint control geometry, and number-format consistency; TASK-100 now follows that remediation batch as well as TASK-095. `TASK-057` repaired the earlier release findings and passed independent review. No deployment/submission readiness is claimed.

The current app implements the approved regional story while preserving the accepted fullscreen composition, dark-ocean palette, typography, 22 evidence marks, native scroll, accessibility behavior, controls, selected-place panel, methods/sources, URL/history, and Explore handoff. Its four stable scenes are `what-the-records-show`, `twenty-two-pacific-places`, `different-directions`, and `unequal-visibility`; Explore starts in explicit neutral `view=overview`. Selecting a place now adds one inline three-strip regional lens using already loaded records; it adds no route, dashboard, renderer, dependency, or new data product. The accepted replacement baseline lives under `artifacts/design/task-077/`; TASK-095 evidence lives under `artifacts/design/task-095/` and TASK-099 responsive evidence under `artifacts/design/task-099/`. TASK-090's boards remain schematic concept references; the shipped retrofit is the bounded, evidence-safe translation recorded by TASK-091 through TASK-094.
...
1. Obtain owner approval for the corrected TASK-101 through TASK-104 batch, then implement and independently review it in order. TASK-101 begins with the required 22-place political-status source review; do not merely hide the workflow marker.
2. Have the owner complete TASK-095's live visual/interaction review; if accepted, record `in-review -> done` without rewriting its frozen evidence package. Append the remediation commit list when the batch closes.
3. Keep TASK-097 through TASK-099 frozen; their implementation, responsive evidence, and independent reviews are complete.
4. After TASK-095 and TASK-101 through TASK-104 close, run TASK-100 frozen independent and owner QA.
5. Treat deployment, public-URL durability, final disclosure, and submission-form actions as a separate release decision after TASK-100.
...
- Final lens-batch assets are 94,930-byte CSS and 1,033,799-byte JavaScript. The checker currently compares those raw files with 95,000/1,050,000-byte internal regression targets. The CSS number is not a Challenge rule: TASK-057 derived it from the 93,895-byte TASK-056 baseline plus 1.18% slack. Measure raw and gzip deltas, but do not compromise TASK-101 through TASK-104 correctness or accessibility to preserve 70 bytes; any justified checker change must be explicit and synchronized with active context.
- TASK-101 through TASK-104 are pending. The first corrects the public political-status path only after current source verification; the second prevents synchronous WebGL failure or context loss from blanking the application or leaving false accessible descriptions; the third covers live 880px breakpoint crossings that fixed-width reload QA misses; the fourth shares signed-number formatting and removes only proven-dead plumbing. None authorizes a palette, figure, mark, data-meaning, state, route, renderer, or dependency redesign.
