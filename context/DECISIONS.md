...
## 2026-08-02: Keep The Lens, Correct Its Typographic Rhythm And Inspection Grammar

Owner review of the completed TASK-107 through TASK-110 batch accepts the inline lens's analytical structure, exact values, independent continuous strips, six-group visibility tally, and informational-only peer inspection. It does not accept the current text hierarchy and spacing as final. Select the refined **Direction A+** response: one centered 320px measure column, 24px selected numbers, 12.5px serif metric names, quiet 11px units and periods, baseline-aligned flex readouts, clear between-measure whitespace, 10px SVG annotations, and a stable two-line visibility inspector. A measured same-state screenshot may reduce the title once to 12px if 12.5px competes with the selected value.

Shorten the visible labels to **Safely managed drinking water**, **Renewable energy share**, and **Reviewed datasets with a record**. Attach each continuous clock to its selected value as **Change · firstYear–latestYear**; clarify default applicability as **N places with comparable change · M unavailable**; say that 12 of 22 places **have records in** all 14 reviewed datasets. Preserve complete scientific wording in accessible summaries and Methods. Tests are updated to protect the better copy rather than used as a reason to keep awkward hyphenated labels.

Give zero, median, inspection, group inspection, and selected place distinct existing-token treatments: thin solid, sparse dash, heavier non-teal solid, non-teal solid outline, and unfilled teal ring. Identify self-inspection as **Selected place**. At 320px, enlarge only the transparent hit bands from 44 to 50 viewBox units so scaling retains at least a 44px CSS interaction target; keep both viewBoxes, visible coordinates, plots, scales, values, and interaction algorithms unchanged.
...
## 2026-08-18: Use An Explicit Places Action And The Existing Panel

Select the explicit **Places** action/panel direction from TASK-117. The action lives in the already visible neutral Explore map header; it opens the existing dock with title **Places**, focuses one native 44px select built from loaded stable code/name pairs, and routes the chosen code through `handleSelect`. Opening is an ephemeral disclosure with no URL entry. Its still-visible button owns `aria-expanded` and `aria-controls="explorer-panel-dock"`; reactivation toggles the panel closed and retains/restores button focus, while panel Close also restores the trigger. One local `closePlaces` helper clears the disclosure without trigger restoration before every canonical transition away—map/`handleSelect`, score, coverage, rank ranges, outlook, guided tour, Methods, and popstate/Back-Forward—with popstate clearing before URL hydration. Selection then hands focus to the selected panel. Direct selected URLs and diagnostic Back/Close/history remain unchanged.

Reject the permanent header select even though it needs fewer lines: live prototypes add 81px at desktop, portrait, and the 320px floor and 76px in landscape. Reject continued map-only selection because it leaves named-place retrieval unresolved. Reject the hidden null-panel chooser because it is not mounted and would inherit the wrong title and permanent panel cost. The chosen action is not layout-free: its third action wraps the portrait and 320px header from 158px to 209px (+51px), while desktop remains 160px and landscape remains 145px. It still yields the smaller defensible product footprint—30px shorter than the permanent select at those compact widths, no standing form, and no dock until requested—so the product remains an atlas rather than a dashboard.

TASK-117 is design-only and follows the owner's latest sequence before the held TASK-110/TASK-100 gates; this supersedes its older post-TASK-100 design dependency without moving production work forward. TASK-118 is the only implementation successor and remains pending until those held release gates are resolved. Reuse the existing panel, native platform control, and one local clearing helper; add no custom combobox, search library, route, alternate selection state, disclosure state machine, second drawer, peer navigation, camera behavior, data output, or visual reskin.

Reason: the map is the primary interface, but a reader who knows a place name should not have to hunt overlapping Pacific labels. A small, explicit action solves discoverability without a standing form or permanent panel; its measured 51px compact-header cost is accepted and visible rather than hidden. The existing panel is already the tested responsive, focus, and recovery surface; using it on demand is a better product simplification than minimizing source lines at the expense of the first view.
