...
The repository is initialized as a context-first GIS/data-science project. `TASK-001` through `TASK-005` are complete: nine priority official datasets have been profiled, contracted, cached, normalized, scored into a baseline Adaptation Gap Index, stress-tested with an app-optional outlook baseline, and exported into enriched app-ready JSON/GeoJSON. The core EDA/story sprint is complete, `TASK-020` records a Dataviz Inspiration audit, TASK-019 evidence-fingerprint divergence artifacts exist, and the React/Vite app now opens as a scroll-led guided atlas with a free-explore handoff backed by generated public app data. `TASK-026`, `TASK-029`, `TASK-028`, `TASK-027`, `TASK-030`, `TASK-031`, `TASK-032`, `TASK-033`, `TASK-034`, `TASK-035`, `TASK-036`, `TASK-037`, `TASK-038`, `TASK-039`, `TASK-040`, `TASK-041`, and `TASK-042` are complete: the map surface uses MapLibre with centroid fallback points, Natural Earth visual land context, first-render graticule lines, overlay labels, accessible geography hit targets, revised evidence-backed story copy, post-map interaction polish, readiness packaging notes, Codex accessibility QA, accepted final Fable visual polish, humanized storyboard copy, desktop guided-story navigation cleanup, island-anchored viewfinder selection, regrouped selected-place detail panel, selected-place nearest-neighbor JSD, nearest-centroid island grouping, encoded island-shaped score/status marks, fixed explainer state, single-sourced/humanized JSD text, and cleaned legend/comparator/fallback mark behavior.
...
1. Start `TASK-043` first. It is the root visual fix from the folded design critique: restore a guaranteed-size presence mark so atoll geographies remain legible while Natural Earth island geometry stays secondary texture/context.
2. After `TASK-043`, run `TASK-044` evidence-bearing motion and `TASK-045` chrome/type cleanup. These can likely be delegated separately if file ownership is split cleanly.
...
- The post-TASK-042 design critique was folded into `TASK-043` through `TASK-047`; the standalone critique file was removed. Core durable finding: literal island polygons under-ink atolls at basin zoom, so the next map mark should be a guaranteed-size presence mark with land geometry as secondary texture/context.
