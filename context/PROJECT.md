...
| Post-TASK-042 design direction | active | `TASK-043` completed presence marks and `TASK-044` completed evidence-bearing motion; `TASK-045` through `TASK-047` remain for chrome/type cleanup, story tightening, and selected-only similarity-arc exploration |
...
- Completed `TASK-044` evidence-bearing motion: score/layer re-encodes and selected focus now move through native MapLibre/CSS transitions with reduced-motion zeroing. Next work starts with `TASK-045` chrome/type cleanup.
