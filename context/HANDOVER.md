...
The repository is a context-first GIS/data-science project. `TASK-001` through `TASK-094`, `TASK-096` through `TASK-109`, and `TASK-111` through `TASK-119` except TASK-118 are complete. Frozen-production TASK-095 has passed automated and independent QA and remains `in-review` for owner visual/interaction acceptance. The inline regional lens now includes the accepted selected reading, evidence-entry hierarchy, correct continuous water/renewable inspectors, a six-group visibility tally, informational-only peer detail, distinct inspection grammar, and at least 44.37px interaction bands at the 320px floor. TASK-119 independently repaired the selected-place desktop map-failure occlusion without changing normal map behavior. TASK-110 attempt 3's evidence-only correction is `in-review`: 13 interaction, 14 responsive/accessibility, 12 contrast, and 10 source-to-render cases all pass against frozen accepted head `6a847f5`; the selected-null camera/target deltas are zero after genuine idle stabilization and the portrait visibility inspection is now visibly framed. Independent re-review and owner acceptance are still required before `done`. TASK-100 remains held until TASK-095/TASK-110 owner gates close. Design-only TASK-117 has independently accepted one visible **Places** disclosure using the existing panel; TASK-118 is a pending post-QA implementation successor. The verified build is 1,045,015-byte JavaScript and 96,839-byte CSS. No deployment/submission readiness is claimed.

The current app implements the approved regional story while preserving the accepted fullscreen composition, dark-ocean palette, typography, 22 evidence marks, native scroll, accessibility behavior, controls, selected-place panel, methods/sources, URL/history, and Explore handoff. Its four stable scenes are `what-the-records-show`, `twenty-two-pacific-places`, `different-directions`, and `unequal-visibility`; Explore starts in explicit neutral `view=overview`. Selecting a place adds one inline regional lens using already loaded records. TASK-114 through TASK-116 complete the release-critical editorial closure without a route, dashboard, renderer, dependency, public-data, model, or global-state change. TASK-117 changes no runtime: its real-shell captures, two concept directions, reachability audit, state/focus/history table, Ponytail comparison, and recommendation live under `artifacts/design/task-117/`. The corrected contract records the Places action's real 51px compact-header cost, button disclosure semantics, deterministic trigger/Close focus, and one-helper clearing invariant across every canonical transition. The accepted replacement baseline lives under `artifacts/design/task-077/`; TASK-095 evidence lives under `artifacts/design/task-095/`, TASK-099 responsive evidence under `artifacts/design/task-099/`, and current TASK-110 attempt-3 evidence under `artifacts/design/task-110/`.
...
1. Independently re-review TASK-110 attempt 3's corrected idle-camera record and visibly framed portrait inspection, then obtain owner visual/interaction acceptance. Keep TASK-095's separate owner gate outstanding.
2. Begin TASK-100 only after TASK-095 and TASK-110 close. Treat deployment, public-URL durability, final disclosure, and submission-form actions as a separate release decision after TASK-100.
3. Only after the held TASK-110/TASK-100 release gates, begin bounded TASK-118 through the existing header/panel/`handleSelect` seams. TASK-117's prototypes are design evidence, not runtime assets.
...
- TASK-113 through TASK-116 do not reopen those plot decisions. They add editorial connective tissue while preserving the current system-sans/Georgia roles, Night Watch palette, data, marks, plots, interaction grammar, and panel architecture. TASK-117 is design-only: it rejects the unmounted neutral-panel chooser and the permanent header select, then recommends one explicit **Places** disclosure with the existing panel. The action preserves 160px desktop/145px landscape headers but wraps compact portrait and 320px headers from 158px to 209px; it requires `aria-expanded`, `aria-controls`, deterministic trigger/Close focus, and one local clearing helper across canonical transitions. TASK-118 remains after held QA and may not add a custom combobox, search dependency, route, alternate selection state, disclosure state machine, second drawer, or dashboard shell. Global font assets, a second gaps-only figure, a claims carousel/scene, and copy-link UI remain deferred.
