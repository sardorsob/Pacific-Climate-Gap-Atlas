...
The repository is a context-first GIS/data-science project. `TASK-001` through `TASK-094` and `TASK-096` through `TASK-109` are complete. Frozen-production TASK-095 has passed automated and independent QA and remains `in-review` for owner visual/interaction acceptance. The readable inline regional lens now has prominent selected readouts, continuous water/renewable inspectors, a six-group visibility tally, and informational-only peer detail. Frozen TASK-110 scientific/UX/accessibility QA passes 802 usable assertions and 10 inspected frames and is also `in-review` for owner acceptance. Its one reproduced failure-layout defect was repaired and independently closed under TASK-102 attempt 3. TASK-100 waits for both owner gates. No deployment/submission readiness is claimed.

The current app implements the approved regional story while preserving the accepted fullscreen composition, dark-ocean palette, typography, 22 evidence marks, native scroll, accessibility behavior, controls, selected-place panel, methods/sources, URL/history, and Explore handoff. Its four stable scenes are `what-the-records-show`, `twenty-two-pacific-places`, `different-directions`, and `unequal-visibility`; Explore starts in explicit neutral `view=overview`. Selecting a place adds one inline regional lens using already loaded records; the completed readability refinement remains inside that component/model/CSS seam and adds no route, dashboard, renderer, dependency, public data, or global state. The accepted replacement baseline lives under `artifacts/design/task-077/`; TASK-095 evidence lives under `artifacts/design/task-095/`, TASK-099 responsive evidence under `artifacts/design/task-099/`, and TASK-110 evidence under `artifacts/design/task-110/`. TASK-090's boards remain schematic concept references; the shipped retrofit is the bounded, evidence-safe translation recorded by TASK-091 through TASK-094.
...
1. Have the owner review TASK-110's 10-frame regional-lens package and live interaction, then move it from `in-review` to `done` only on explicit acceptance.
2. Have the owner complete TASK-095's still-outstanding Night Watch live review.
3. Begin TASK-100 only after both owner gates close; reuse their frozen evidence and return any defect to its owning task.
4. Treat deployment, public-URL durability, final disclosure, and submission-form actions as a separate release decision after TASK-100. Keep completed TASK-105 frozen as one static public asset.
