...
1. Start `TASK-047` next: test whether selected-only JSD neighbor arcs add enough story value to justify more map complexity now that `TASK-046` kept JSD as a late selected-detail beat.
2. If the arcs read as physical connection, causality, migration, shared risk, or a decorative web, document the no-build decision and keep similarity in the selected-place panel.
