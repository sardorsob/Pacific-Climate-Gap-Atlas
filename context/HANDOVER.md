...
The current app remains the functioning behavioral/scientific baseline. Owner-approved `TASK-058` replaces the rail composition with **One Constellation on an Elastic Stage**: a fullscreen premise and map-led scenes 1–3, fullscreen Nauru/Tuvalu and rank-band takeovers for scenes 4 and 5, and a return of the same marks to geography for Explore. `TASK-059` is done and `TASK-060` is in review with the fullscreen sticky shell and a focused automatic reload-restoration correction; root must rerun the exact landscape-to-portrait copied-scene reload plus the remaining desktop/mobile/landscape matrix before `TASK-061` starts. Three concept boards live under `artifacts/design/task-058/`; their generated content is composition-only. The written contract is consolidated in `context/ARTISTIC_REDESIGN_BRIEF.md`.
...
1. Complete root browser/Checker QA for `TASK-060`, then execute `TASK-061` through `TASK-064` from `context/plans/tasks-059-064-fullscreen-story-stage-implementation-plan.md`, one task/commit at a time with separate Maker and Checker passes.
...
- `TASK-060` reuses that observer and sticky map for fullscreen ownership, omits premise top/progress chrome from the DOM, ignores story keys from interactive controls, and publishes active visual/stage attributes. Its hydration flow now sets manual history scroll restoration so a retained landscape reload offset cannot overwrite the copied-scene scroll after a portrait switch. Root still needs to rerun that exact reproduction and verify scroll/history/resize/focus/reduced-motion behavior at the target viewports before acceptance.
