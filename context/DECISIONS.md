...
Select the explicit **Places** action/panel direction from TASK-117. The action lives in the already visible neutral Explore map header; it opens the existing dock with title **Places**, focuses one native 44px select built from loaded stable code/name pairs, and routes the chosen code through `handleSelect`. Opening is an ephemeral disclosure with no URL entry. Close restores its trigger; selection hands focus to the selected panel; popstate closes the disclosure before existing URL hydration. Direct selected URLs and diagnostic Back/Close/history remain unchanged.

Reject the permanent header select even though it needs fewer lines: live prototypes add 76–81px to the map header across desktop, portrait, landscape, and the 320px floor. Reject continued map-only selection because it leaves named-place retrieval unresolved. Reject the hidden null-panel chooser because it is not mounted and would inherit the wrong title and permanent panel cost. The chosen action adds the smallest default visual footprint and keeps the product an atlas rather than a dashboard.

TASK-117 is design-only and follows the owner's latest sequence before the held TASK-110/TASK-100 gates; this supersedes its older post-TASK-100 design dependency without moving production work forward. TASK-118 is the only implementation successor and remains pending until those held release gates are resolved. Reuse the existing panel and native platform control; add no custom combobox, search library, route, alternate selection state, second drawer, peer navigation, camera behavior, data output, or visual reskin.

Reason: the map is the primary interface, but a reader who knows a place name should not have to hunt overlapping Pacific labels. A small, explicit action solves discoverability without permanently shrinking or form-loading the neutral atlas. The existing panel is already the tested responsive, focus, and recovery surface; using it on demand is a better product simplification than minimizing source lines at the expense of the first view.
