...
| Post-TASK-042 design direction | active | `TASK-043` completed presence marks, `TASK-044` completed evidence-bearing motion, and `TASK-045` completed chrome/type cleanup; `TASK-046` and `TASK-047` remain for story tightening and selected-only similarity-arc exploration |
...
- Completed `TASK-045` chrome/type cleanup: the app now uses an intentional system type stack, removes the low-value explore metrics strip, and quiets remaining floating map chrome while preserving detail-panel readability and all data/method caveats. Next work starts with `TASK-046` guided-story tightening.
