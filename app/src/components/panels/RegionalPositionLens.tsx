...
function Strip({ strip, name, clock }: { strip: RegionalPositionStrip; name: string; clock: string | null }) {
  ...
  return (
    <section className="regional-lens__strip" data-regional-strip={strip.id} aria-label={summary}>
      <div className="regional-lens__label">
      ...
      {extent && scale && (
        <svg className="regional-lens__plot" viewBox="0 0 320 46" aria-hidden="true">
          ...
          {strip.applicable.map((observation) => (
            <circle key={observation.code} data-peer-mark className="regional-lens__peer" cx={position(observation.value, scale)} cy={20 - observation.lane * 2} r="2.8" aria-hidden="true" />
          ))}
          ...
          <text x="16" y="44" textAnchor="start" aria-hidden="true">{valueLabel(strip, extent[0])}</text>
          <text x="304" y="44" textAnchor="end" aria-hidden="true">{valueLabel(strip, extent[1])}</text>
        </svg>
      )}
      <p className="regional-lens__meta">
...
export function RegionalPositionLens({ geo, geos }: { geo: Geo; geos: Geo[] }) {
  return <section className="regional-lens" aria-labelledby="regional-position-heading">
    ...
    {buildRegionalPositionModel(geos, geo.code).map((strip) => {
      ...
      const clock = measure && measure.firstYear !== null && measure.latestYear !== null ? `${measure.firstYear} to ${measure.latestYear}` : null;
