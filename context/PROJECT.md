...
| Post-TASK-042 design direction | active | `TASK-043` completed presence marks, `TASK-044` completed evidence-bearing motion, `TASK-045` completed chrome/type cleanup, and `TASK-046` completed guided-story tightening; `TASK-047` remains for selected-only similarity-arc exploration |
...
- Completed `TASK-046` guided-story tightening: the story now opens on what official records can show, brings thin monitoring records before the formula, and keeps JSD as a late selected-detail beat rather than a hook or global layer. Next work starts with `TASK-047` selected-only neighbor arcs.
