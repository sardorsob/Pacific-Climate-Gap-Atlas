...
- Functional notes: The live audit confirms that `panelOpen` is false in neutral Explore, the `CountryPanel` null branch is unmounted, broadening it would reserve 400px and inherit **Rank ranges**, and `handleSelect` is the canonical selection/history path. Compare a permanent native header select with one explicit **Places** action that opens the existing panel, while retaining map-only selection as the no-change baseline and rejecting the hidden chooser. Select the explicit action/panel: it keeps the neutral map cleaner, opens a correctly titled/focused panel only on request, uses one native loaded-code select, and needs no new route, selected state, dependency, drawer, or camera behavior. This task changes design records and evidence only; TASK-118 owns any later implementation after held release QA.
...
- Acceptance criteria: The chosen host is actually rendered in neutral Explore, preserves map prominence, has a complete desktop/mobile/focus/history/recovery contract, and can reuse `handleSelect` without hidden parallel state. The decision is implementation-ready but remains behind TASK-110/TASK-100. No production, test, data, dependency, route, state, lockfile, or runtime asset changes during TASK-117; independent UX/accessibility review passes before `done`.
...
- QA notes: Source trace proves the six required host facts. A dedicated quiet headless Chromium pass captured four real neutral shells and inspected selected, coverage-root, and uncertainty-child states at all four sizes with 22 map targets, correct panel titles/Back/Close behavior, zero page overflow, and zero console/page errors. Matched live-app-derived desktop/mobile concepts and all-four-size geometry show the permanent header select adds 76–81px to the header, while the explicit Places panel retains the baseline header and reserves the existing 400px/mobile sheet only after activation. Both native selects measure 44px. Structured audit, state/focus/history contract, Ponytail comparison, recommendation, screenshots, and measurements are retained under `artifacts/design/task-117/`. Production/tests/data/dependencies/routes/state/lockfiles/runtime assets remain untouched. Independent UX/accessibility review owns `done`.
- Attempts: 1
- Max attempts: 3
- Attempt log: 2026-08-18: TASK-117 moved `pending -> in-progress` under the owner's latest instruction to finish the delegated batch while TASK-100/TASK-110 remain held. The live source/shell audit rejected the hidden panel, compared both reachable directions in the actual Night Watch app, selected the explicit Places action/panel, registered bounded post-QA TASK-118, retained the complete evidence package, and moved `in-progress -> in-review`. No runtime or data change was made.
- Status: in-review
...
- Functional notes: Add one 44px **Places** action to the existing neutral Explore header. Show it only when no selected or diagnostic panel owns the dock. On activation, use one local ephemeral `placesOpen` boolean and trigger ref to open the existing dock with title **Places**, exact approved copy, and one native labelled select sorted by reviewed name while retaining stable codes as values. Focus the select on open. Close without a URL write and restore the trigger. On selection, close the picker, call the existing `handleSelect`, and focus the existing panel Close control; do not write another URL or selected state. Close the disclosure before popstate hydration. Direct selected URLs and coverage/uncertainty root/child Back/Close/history behavior remain unchanged. Loading/error/zero-loaded-record states keep the current app-wide gate and do not mount an empty picker.
...
- Edge cases: All 22 names; Federated States of Micronesia and Northern Mariana Islands; neutral/selected/direct-URL/coverage/uncertainty states; opening/Close/select/popstate; keyboard and touch; desktop, portrait, 844x390 landscape, 320x568, reflow; loading/error/zero records; focus removal; no horizontal overflow.
- Test cases: Begin with focused RED assertions for action reachability only in neutral Explore, exact **Places** title/copy, loaded alphabetical stable code/name options, 44px native control, open focus, Close focus restoration without URL write, selection delegation to `handleSelect`, selected-panel focus after option activation, and popstate closure. Keep existing direct URL, diagnostic Back/Close, sheet expansion/collapse, Methods, map selection, story, and URL-state tests green. Mutate the select handler to bypass `handleSelect` and the Close focus target to prove the new assertions are meaning-bearing.
...
- Artifacts to produce: Neutral/open/selected/Close states at 1440x900, 390x844, 844x390, and 320x568; option order/code trace; focus/history/popstate trace; control/overflow measurements.
- Acceptance criteria: The Places action is visible and understandable in neutral Explore; the picker uses the existing panel and native control; all 22 reviewed names are reachable; focus never falls to the document; existing selection/URL/history/diagnostic behavior remains canonical; neutral map prominence returns after Close/selection; 44px and zero-overflow contracts pass; no dependency, route, data, custom combobox, second drawer, panel stack, camera controller, alternate selected state, dashboard shell, or story change enters; independent review passes.
...
- Manual QA: Quietly exercise open/Close/select/Back/Forward/popstate across desktop, portrait, landscape, and 320px; verify focus, full long names, 44px targets, existing selected/diagnostic panels, map prominence, and zero overflow/runtime errors.
