...
The repository is a context-first GIS/data-science project. `TASK-001` through `TASK-094`, TASK-096 through TASK-099, and TASK-101 through TASK-106 are complete. Frozen-production TASK-095 has passed automated and independent QA and remains `in-review` for owner visual/interaction acceptance. The first inline regional lens is implemented and independently approved, but owner use found its information hierarchy difficult to decipher. TASK-106 records the approved refinement: prominent selected readouts, continuous water/renewable inspectors, a six-group visibility tally, and informational-only peer detail. TASK-107 through TASK-110 are pending and sequential; TASK-100 now waits for TASK-110 as well as TASK-095. The completed QA-remediation batch and TASK-105 favicon remain frozen. `TASK-057` repaired the earlier release findings and passed independent review. No deployment/submission readiness is claimed.

The current app implements the approved regional story while preserving the accepted fullscreen composition, dark-ocean palette, typography, 22 evidence marks, native scroll, accessibility behavior, controls, selected-place panel, methods/sources, URL/history, and Explore handoff. Its four stable scenes are `what-the-records-show`, `twenty-two-pacific-places`, `different-directions`, and `unequal-visibility`; Explore starts in explicit neutral `view=overview`. Selecting a place adds one inline regional lens using already loaded records; the pending refinement remains inside that component/model/CSS seam and adds no route, dashboard, renderer, dependency, public data, or global state. The accepted replacement baseline lives under `artifacts/design/task-077/`; TASK-095 evidence lives under `artifacts/design/task-095/` and TASK-099 responsive evidence under `artifacts/design/task-099/`. TASK-090's boards remain schematic concept references; the shipped retrofit is the bounded, evidence-safe translation recorded by TASK-091 through TASK-094.
...
1. Implement TASK-107 first: extend only the pure regional-position model/tests with source-derived peer clocks and deterministic exact-value groups.
2. Implement TASK-108 and TASK-109 sequentially because both own `RegionalPositionLens.tsx`, its tests, and the same small CSS section: continuous readout/inspection first, grouped visibility tally second. Commit and review each task separately.
3. Freeze production for TASK-110's scientific, visual, responsive, interaction, and accessibility matrix; obtain owner acceptance before moving it to `done`.
4. Have the owner complete TASK-095's still-outstanding Night Watch live review. TASK-100 begins only after both TASK-095 and TASK-110 are accepted.
5. Treat deployment, public-URL durability, final disclosure, and submission-form actions as a separate release decision after TASK-100. Keep completed TASK-105 frozen as one static public asset.
...
- Current built assets are 94,842-byte CSS and 1,035,371-byte JavaScript. The checker compares those raw files with 95,000/1,050,000-byte internal regression targets. The CSS number is not a Challenge rule: TASK-057 derived it from the 93,895-byte TASK-056 baseline plus 1.18% slack. Measure raw and gzip deltas, but do not compromise correctness or accessibility to preserve a few bytes; any justified checker change must be explicit and synchronized with active context.
