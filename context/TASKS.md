...
- QA notes: Initial Builder evidence remains in commit `0a7c6bc`; first repair is `13265f1`. Final re-review requested three cache-boundary fixes only. Attempt-3 TDD proves the fetcher writes the exact CRLF payload bytes used for manifest count/hash, the fallback live-cache path also uses bytes, invalid UTF-8 becomes `cache_manifest_error`, and an existing manifest with a missing file or missing entry blocks both profiler and processed-path live fallback. Fresh live acquisition passed 15/15 plus 1/1 supplementary, profiling passed 15/15, focused tests passed 18/18, and full Python passed 75/75 with only pre-existing pandas FutureWarnings. The nine-source rebuild remains 14,007 value-equivalent rows across 22 geographies; tracked processed/app/index/story artifacts remain unchanged. Independent QA still owns `done`.
- Attempts: 3
- Max attempts: 3
