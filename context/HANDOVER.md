...
The repository is initialized as a context-first GIS/data-science project. `TASK-001` through `TASK-056`, `TASK-058` through `TASK-063`, `TASK-065` through `TASK-067`, and `TASK-070` are complete. `TASK-064` remains in owner review and `TASK-057` remains `needs-fix`. `TASK-066` expands processed research data to 16,410 rows across 14 datasets without changing the baseline index, public app, or story. Owner QA accepted the `TASK-067` audit of the 2,403 candidate rows. The owner then approved a regional two-act working hypothesis—cross-currents followed by unequal evidence visibility, then release into the existing explorer—and requested `TASK-068` as a deeper complete-region EDA gate. `TASK-069` makes the final story decision; `TASK-071` rewrites the roadmap after approval.
...
This writes the script-first EDA tables under `artifacts/tables/`, records `artifacts/provenance/eda_summary.json`, and produces the TASK-067 accepted-candidate tables, five static analytical research figures, and one separate contact sheet under `artifacts/figures/`. It also includes coverage deep dives, indicator forensics, country story labels, rank volatility, trend profiles, monitoring-gap GIS story priorities, and TASK-019 Evidence Fingerprint Divergence outputs. TASK-068 will extend the same command with five regional tables, six research plates, and one deterministic file-based run bundle. Read `context/ANALYSIS_BRIEF.md`, `context/DATA_CARD.md`, `context/EXPERIMENTS.md`, and the TASK-067 atlas before implementing TASK-068; none of these figures is final scene art.

## Next Recommended Work

1. Implement `TASK-068` with RED tests, the smallest reviewed transforms, six full-size research plates, separate condition/visibility ordering, sensitivity evidence, deterministic hashes, and a valid run bundle.
...
- TASK-068 must keep condition and visibility matrices separate, preserve all missing cells, flag derived-input circularity, and treat hierarchical clustering as exploratory ordering only. Coverage or monitoring must not be described as emergency preparedness.
